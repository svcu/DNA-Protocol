...
app.post("/add-check", (req, res)=>{
    ...
    if(!signature){
        ...
        if(balance>=obj.amount && myHash==chainHash && validate && obj.amount!==0 && verify){

            let r = DNAChain.addNoBroadcast(obj);

            res.send(r)

        }else if(obj.amount == 0 && DNAChain.currentStrand==0 && balance>=obj.amount && myHash==chainHash && validate && verify){
